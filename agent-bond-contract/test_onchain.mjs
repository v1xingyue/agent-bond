...
function buildInitializeIx(admin, treasuryPk) {
  const [configPda] = PublicKey.findProgramAddressSync([Buffer.from("config")], PROGRAM_ID);
  const payload = Buffer.alloc(32);
  treasuryPk.toBuffer().copy(payload);
...
function buildDepositIx(userPk, amountLamports) {
  const [configPda] = PublicKey.findProgramAddressSync([Buffer.from("config")], PROGRAM_ID);
...
function buildExecuteSkillIx(userPk, agentId, skillId, cost, bondGain) {
  const [configPda] = PublicKey.findProgramAddressSync([Buffer.from("config")], PROGRAM_ID);
  const [userStatePda] = PublicKey.findProgramAddressSync(
    [Buffer.from("user_state"), userPk.toBuffer()],
    PROGRAM_ID
...
async function main() {
  ...
  // 2. Deposit 0.5 SOL
  console.log("\n2. Deposit 0.5 SOL...");
  const depositIx = buildDepositIx(user.publicKey, BigInt(Math.floor(0.5 * LAMPORTS_PER_SOL)));
